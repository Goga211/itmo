...
`primary` сам выполнит [`init-primary.sh`](./primary/init/init-primary.sh) на первом старте, создаст пользователя `replicator` и скопирует `postgresql.conf`/`pg_hba.conf` в `PGDATA`.
