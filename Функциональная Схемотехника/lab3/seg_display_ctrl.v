// Контроллер динамической индикации семисегментного дисплея
module seg_display_ctrl (
    ...
    // Счётчик для формирования частоты мультиплексирования
    // При clk = 50 МГц и делителе 50_000 получаем 1 кГц.
    // 8 разрядов -> частота обновления ~125 Гц на разряд — комфортно.
    reg [15:0] scan_cnt;
    ...
    always @(*) begin
        // Проверяем нужно ли гасить этот разряд
