...
module fec_saylinx_board_top_tb;
    ...
    //==================================================================
    // Task запуска тестового сценария
    ...
    begin
        ...
        $display("[CHECK] After KEY2: a_value = %0d, display_mode = %0d", 
                 dut.a_value, dut.display_mode);
        
        // Показать 'b'
        press_key4;
        $display("[CHECK] After KEY4: b_value = %0d, display_mode = %0d", 
                 dut.b_value, dut.display_mode);
        ...
        $display("[RESULT] last_result = %0d (0x%04h)", dut.last_result[15:0], dut.last_result[15:0]);
        $display("[RESULT] display_value = 0x%08h", dut.display_value);
        
        if (dut.last_result[15:0] == expected) begin
            $display("[PASS] Test PASSED: Result = %0d", dut.last_result[15:0]);
        end else begin
            $display("[FAIL] Test FAILED: Expected %0d, got %0d", 
                     expected, dut.last_result[15:0]);
        end
    ...
    initial begin
        ...
        // ========= ТЕСТ 1 =========
        // 5 * cuberoot(27) = 5 * 3 = 15
        run_case(8'd5, 8'd27, 16'd15);

        // ========= ТЕСТ 2 =========
        // 10 * cuberoot(8) = 10 * 2 = 20
        run_case(8'd10, 8'd8, 16'd20);

        // ========= ТЕСТ 3 =========
        // 7 * cuberoot(64) = 7 * 4 = 28
        run_case(8'd7, 8'd64, 16'd28);
    ...
    always @(posedge CLK) begin
        if ($time % 10000000 == 0) begin  // Каждые 10 ms
            $display("%0.3f\t%b\t%0d\t%0d\t%0d\t%0d",
                     ...
                     dut.a_value, dut.b_value, dut.calc_result);
        end
    end
