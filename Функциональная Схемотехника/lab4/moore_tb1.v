...
module moore_tb1;
    ...
    localparam integer MAX_WAIT_CYCLES = 20000; // Увеличено для медленных вычислений
    ...
    task automatic run_case(
        ...
        begin
            ...
            if (!done) begin
                $display("%2d: TIMEOUT after %0d cycles  a=%0d, b=%0d", num, w, ta, tb);
                $display("    DEBUG: state=%0d, cub_busy=%0d, mult_busy=%0d", 
                         dut.state, dut.cub_busy, dut.mult_busy);
            end else begin
                @(posedge clk);
                $display("%2d: Completed in %0d cycles", num, w);
                if (result === expected)
                    $display("    %0d*cuberoot(%0d) = %0d  [OK]",   ta, tb, result);
                else
                    $display("    %0d*cuberoot(%0d) = %0d  exp %0d  [FAIL]",
                             ta, tb, result, expected);
            end
